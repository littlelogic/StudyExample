...
public class AlbumHelper {
    ...
    private void getThumbnail() {
        ...
        Cursor cursor = null;
        try {
            cursor = contentResolver.query(Thumbnails.EXTERNAL_CONTENT_URI,projection, null, null, null);
    ...
    private void buildImagesBucketList() {
        ...
        try {
            ...
            // 得到一个游标
            cur = contentResolver.query(Media.EXTERNAL_CONTENT_URI, columns, null, null, null);
            if (null == cur){
                return;
            }
            if (cur.moveToFirst()) {
                ...
                do {
                    ...
                    //TODO 加快比较-改
                    if(lastBucketId.equals(bucketId)){
                    }else{
                        lastBucket  = bucketList.get(bucketId);
                        if (lastBucket == null) {
                            lastBucketId=bucketId;
                            lastBucket = new ImageBucket();
                            bucketList.put(bucketId, lastBucket);
                            lastBucket.imageList = new ArrayList<ImageItem>();
                            lastBucket.bucketName = bucketName;
                            lastBucket.bucketId = bucketId;
                        }
                    }
                    //----------------
                    lastBucket.count++;
