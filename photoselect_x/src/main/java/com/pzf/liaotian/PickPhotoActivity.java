...
public class PickPhotoActivity extends TitleBarActivity implements
    ...
    private void openImageGridActivity(int position) {
        ...
        intent.putExtra(ConstantKeys.EXTRA_IMAGE_LIST,(Serializable) mDataList.get(position).imageList);
        intent.putExtra(ConstantKeys.EXTRA_ALBUM_NAME,mDataList.get(position).bucketName);
        intent.putExtra(ConstantKeys.EXTRA_ALBUM_ID,mDataList.get(position).bucketId);
        Log.i("wjw02","PickPhotoActivity--openImageGridActivity--bucketId->"+mDataList.get(position).bucketId);
