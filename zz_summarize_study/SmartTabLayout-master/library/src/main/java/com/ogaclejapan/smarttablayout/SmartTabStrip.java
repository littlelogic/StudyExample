...
class SmartTabStrip extends LinearLayout {
  ...
  private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness,
    ...
    if (indicatorCornerRadius > 0f) {
      ...
      m.a();
      int width_fixed = 70;
      if (indicatorRectF.width() != width_fixed) {
        float centreX = indicatorRectF.left + indicatorRectF.width() / 2;
        indicatorRectF.left = centreX - width_fixed / 2;
        indicatorRectF.right = centreX + width_fixed / 2;
      }
      // todo ----------------->
