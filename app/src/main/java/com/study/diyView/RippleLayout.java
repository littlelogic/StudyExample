...
// com.study.diyView.RippleLayout
...
//    private float rippleWidth = 400f;
    //水波扩散速度
...
//                rippleRadius = count / 2 * rippleSpeed;
//                rippleRadius = viewLength / 7;
    ...
    private void initData() {
        ...
        for (int height = 0; height <= MESH_HEIGHT; height++) {
            ...
            for (int width = 0; width <= MESH_WIDTH; width++) {
                ...
                staticVerts[index] = targetVerts[index] = x;
                staticVerts[index + 1] = targetVerts[index + 1] = y;
                index += 2;
            }
        }
    }
    ...
    private PointF getRipplePoint(float originX, float originY, float staticX, float staticY) {
        ...
        //计算偏移距离
        float rate = (float) ((length - rippleRadius) / rippleWidth );
//        float rate = (float) ((length - rippleRadius) / rippleWidth * 2f*Math.PI);
//        float rate = (float) ((length - rippleRadius) / rippleWidth * 0.25f*Math.PI);
//        float offset = (float) Math.cos(rate) * 10f;

//        float offset = (float) Math.sin(rate*Math.PI) * 10f;
        float offset = (float) Math.sin(rate*Math.PI) * 30f;
//        float offset = (float) Math.cos(rate) * 30f;
