package com.example.wujiawen.ExampleDragFillBlankView;//com.example.wujiawen.ExampleDragFillBlankView.DragFillBlankView
 ...
 *
     ...
     不过你要先知道你需要的字符在TextView 是第几个字符  然后用layout.getLineForOffset得到所在行数
     再用 layout.getLineBounds得到 这一行的外包矩形（Rect) 这样 这个字符的顶部Y坐标就是rect的top 底部Y坐标就是rect的bottom
